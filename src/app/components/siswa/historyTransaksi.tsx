...
export const HistoryTransaksi = () => {
  ...
  const handleOpenNota = async (item) => {
    setSelectedTransaksi(item);
    try {
      const res = await detailTransaksiService.getByTransaksiId(item._id);
      setSelectedDetailTransaksi(res?.data || []);
    } catch (err) {
      console.error("Gagal mengambil detail transaksi:", err);
      setSelectedDetailTransaksi([]);
    }
  };
          ...
          {filteredTransaksi?.map((item) => (
            <div key={item._id} className="bg-white shadow-md rounded-2xl p-6 border border-gray-200">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold text-gray-900">ID Transaksi: {item._id}</h2>
